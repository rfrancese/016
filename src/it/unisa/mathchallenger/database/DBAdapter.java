...
public class DBAdapter implements Database {
	...
	private ContentValues cv_Amico(int account_id, int id_amico,
			String friend_user) {
		ContentValues values = new ContentValues();
	...
	private ContentValues cv_Partita(int idPartita, int account,
			int id_sfidato, String sfidato_user, int stato) {
	...
	public void inserisciAccount(int id, String username, String authcode)
			throws SQLException {
		deleteAll("account");
		database.insertOrThrow("account",
				null,
				cv_Account(id, username, authcode));
	}

	public AccountUser selezionaAccount() {
		Cursor res = database.query("account",
				null,
				null,
				null,
				null,
	...
	public void
			inserisciAmico(int account_id, int id_amico, String friend_user) {
		database.insertOrThrow("amici",
				null,
				cv_Amico(account_id, id_amico, friend_user));
	}

	public void rimuoviAmico(int account, int id_amico) {
		database.delete("amici",
				"id_amico=" + id_amico + " AND account=" + account,
				null);
	}

	public ArrayList<Account> selezionaAmici(int account) {
		Cursor res = database.query("amici",
				null,
				"account=" + account,
				null,
				null,
	...
	public void inserisciPartita(int idPartita, int account, int id_sfidato,
			String sfidato_user, int stato) {
		database.insert("partite",
				null,
				cv_Partita(idPartita, account, id_sfidato, sfidato_user, stato));
	}

	public void aggiornaPartita(int idPartita, int account, int id_sfidato,
			String sfidato_user, int newStato) {
		ContentValues cv = cv_Partita(idPartita,
				account,
				id_sfidato,
				sfidato_user,
				newStato);
	...
	public ArrayList<Partita> selezionaPartiteInCorso(int account) {
		Cursor res = database.query("partite",
				null,
				"account=" + account + " AND stato_partita<=" + Partita.INIZIATA,
				null,
				null,
				null,
				null);
