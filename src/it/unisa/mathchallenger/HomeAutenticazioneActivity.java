...
public class HomeAutenticazioneActivity extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (acc != null && comm.isConnected()) {
			Messaggio m = CommunicationMessageCreator.getInstance()
					.createLoginAuthcode(acc.getID(), acc.getAuthCode());
			try {
				comm.send(m);
				loginOK = CommunicationParser.getInstance().parseLoginAuthcode(
						m);
	...
	public void onBackPressed() {
		new AlertDialog.Builder(this).setMessage(R.string.exit_confirm)
				.setCancelable(false)
		...
		if (v.getId() == R.id.button_registrazione_home) {
				...
				public boolean onKey(View v, int keyCode, KeyEvent event) {
					...
					case KeyEvent.KEYCODE_SPACE:
						...
						Toast.makeText(getApplicationContext(),
								R.string.string_contains_spaces,
			...
			user.setOnKeyListener(new EditText.OnKeyListener() {
				...
				public boolean onKey(View v, int keyCode, KeyEvent event) {
					...
					if (s2.length() > 30) {
						...
						new AlertDialog.Builder(HomeAutenticazioneActivity.this)
								.setMessage(
										R.string.invalid_registration_username)
								.setCancelable(true)
			...
			mail.setOnKeyListener(new EditText.OnKeyListener() {
				...
				public boolean onKey(View v, int keyCode, KeyEvent event) {
					...
					if (email.length() > 100) {
						new AlertDialog.Builder(HomeAutenticazioneActivity.this)
								.setMessage(R.string.invalid_registration_mail)
								.setCancelable(true)
								.setNeutralButton("OK", null).show();
			...
			if (pass1.length() < 8) {
				new AlertDialog.Builder(HomeAutenticazioneActivity.this)
						.setMessage(R.string.invalid_registration_password)
			...
			if (pass1.compareTo(pass2) != 0) {
				Toast.makeText(getApplicationContext(),
						R.string.reg_password_diverse, Toast.LENGTH_LONG)
						.show();
				return;
			} else {
				Messaggio m = CommunicationMessageCreator.getInstance()
						.createRegisterMessage(username, pass1, email);
				try {
					comm.send(m);
					AccountUser acc = CommunicationParser.getInstance()
							.parseRegister(m);
					if (acc == null) {
						if (m.hasError()) {
							Toast.makeText(getApplicationContext(),
									m.getErrorMessage(), Toast.LENGTH_LONG)
									.show();
						} else
							Toast.makeText(getApplicationContext(),
									R.string.error_unknown, Toast.LENGTH_LONG)
									.show();
					} else {
					...
					Toast.makeText(getApplicationContext(),
							R.string.communication_error, Toast.LENGTH_LONG)
							.show();
					e.printStackTrace();
	...
	public void onClickResetPassword(View v) {
		if (v.getId() == R.id.button_recupera_pass_home)
			...
			if (user.isEmpty()) {
				...
				Messaggio msg = CommunicationMessageCreator.getInstance()
						.createResetMessage(user);
				try {
					comm.send(msg);
					boolean reset_psw = CommunicationParser.getInstance()
							.parseResetPassword(msg);
					if (reset_psw) {
						Toast.makeText(getApplicationContext(),
								R.string.reset_password_ok, Toast.LENGTH_LONG)
								.show();
					} else {
						Toast.makeText(getApplicationContext(),
	...
	public void onClickAccedi(View v) {
		...
		try {
			Messaggio m = CommunicationMessageCreator.getInstance()
					.createLoginMessage(username, pass);
			comm.send(m);
			AccountUser acc = CommunicationParser.getInstance().parseLogin(m);
			if (acc == null) {
				Toast.makeText(getApplicationContext(),
						R.string.login_fail_message, Toast.LENGTH_LONG).show();
			} else {
				acc.setUsername(username);
				Status.getInstance().login(acc);
			...
			Toast.makeText(getApplicationContext(),
					R.string.communication_error, Toast.LENGTH_LONG).show();
			e.printStackTrace();
		} catch (LoginException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
